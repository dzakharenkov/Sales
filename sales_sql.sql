...
-- Приход: warehouse_receipt, return_from_customer (status=completed), allocation/transfer (warehouse_to).
-- Расход: allocation, delivery, promotional_sample, write_off, damage, transfer (warehouse_from).
-- ============================================================
...
SELECT
  ...
  (COALESCE(SUM(CASE WHEN o.type_code IN ('warehouse_receipt', 'return_from_customer') AND o.status = 'completed' AND (o.warehouse_to = wh.code OR (o.warehouse_to IS NULL AND o.warehouse_from = wh.code)) THEN o.quantity ELSE 0 END), 0)
   + COALESCE(SUM(CASE WHEN o.type_code IN ('allocation', 'transfer') AND o.warehouse_to = wh.code AND (o.status IS NULL OR lower(trim(COALESCE(o.status, ''))) NOT IN ('cancelled','canceled')) THEN o.quantity ELSE 0 END), 0)
   - COALESCE(SUM(CASE WHEN o.type_code IN ('allocation', 'delivery', 'promotional_sample', 'write_off', 'damage', 'transfer') AND o.warehouse_from = wh.code AND (o.status IS NULL OR lower(trim(COALESCE(o.status, ''))) NOT IN ('cancelled','canceled')) THEN o.quantity ELSE 0 END), 0)) AS total_qty
...
HAVING (COALESCE(SUM(CASE WHEN o.type_code IN ('warehouse_receipt', 'return_from_customer') AND o.status = 'completed' AND (o.warehouse_to = wh.code OR (o.warehouse_to IS NULL AND o.warehouse_from = wh.code)) THEN o.quantity ELSE 0 END), 0)
   + COALESCE(SUM(CASE WHEN o.type_code IN ('allocation', 'transfer') AND o.warehouse_to = wh.code AND (o.status IS NULL OR lower(trim(COALESCE(o.status, ''))) NOT IN ('cancelled','canceled')) THEN o.quantity ELSE 0 END), 0)
   - COALESCE(SUM(CASE WHEN o.type_code IN ('allocation', 'delivery', 'promotional_sample', 'write_off', 'damage', 'transfer') AND o.warehouse_from = wh.code AND (o.status IS NULL OR lower(trim(COALESCE(o.status, ''))) NOT IN ('cancelled','canceled')) THEN o.quantity ELSE 0 END), 0)) != 0
...
-- ============================================================
-- Telegram-бот SDS v2.1: таблицы сессий, логов, попыток входа
-- ============================================================

-- Сессии Telegram-бота (одна активная на пользователя)
...
-- Попытки входа (блокировка после 3 неудачных за 10 мин)
