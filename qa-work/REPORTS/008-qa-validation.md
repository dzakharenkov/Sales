# QA Report: 008 ? 008-API-PAGINATION
...
- [ ] All list endpoints accept `limit` (default 50, max 200) and `offset` (default 0) query params
- [ ] All list endpoints return `{"data": [...], "total": N, "limit": 50, "offset": 0}` envelope
- [ ] `total` count is always accurate (uses COUNT query, not len(results))
- [ ] Frontend `app.js` updated to use paginated responses (display page info, prev/next buttons)
- [ ] OpenAPI docs show pagination parameters for all list endpoints
