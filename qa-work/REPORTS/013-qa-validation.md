# QA Report: 013 ? Database Performance Indexes
...
- [ ] All indexes added via Alembic migration (not raw SQL)
- [ ] `EXPLAIN ANALYZE` run before/after each index to verify improvement
- [ ] Index creation done with `CREATE INDEX CONCURRENTLY` to avoid locking production
- [ ] No duplicate indexes (check existing before creating)
