...
- **Статус**: ✅ Исправлено
- **Файл(ы) изменены**: `src/static/app.html`
- **Строки кода**: formatScheduledDelivery, formatScheduledDeliveryOi
- **Что было**: Просроченные даты выделялись красным и для отменённых заказов.
- **Что стало**: Если статус «Отменён» (cancelled, canceled, 4, отменен и т.д.) — дата не выделяется красным; просроченная дата красным только для не отменённых заказов.
- **Проверка**: Заказ со статусом «Отменён» — дата поставки обычным цветом; просроченный незакрытый заказ — дата красная.

## Исправление #13: Поиск позиций заказа — Кнопка открытия
- **Статус**: ✅ Исправлено
- **Файл(ы) изменены**: `src/static/app.html`
- **Строки кода**: loadSectionOrders — tryOpenOrderCard() вызывается и при успешной загрузке раздела (в .then), не только в .catch; data-order-open задаёт _openOrderNo и showSection('orders').
- **Что было**: Кнопка открывала поиск заказов, а не карточку конкретного заказа.
- **Что стало**: По нажатию кнопки в колонке «Заказ» — переход в раздел заказов и открытие формы редактирования именно этого заказа (по order_no).
- **Проверка**: Позиции заказов → нажать кнопку в колонке «Заказ» — открывается карточка выбранного заказа.
...
- **Статус**: ✅ Выполнено
- **Файл(ы) изменены**: `src/api/v1/routers/operations_flow.py`, `src/api/v1/routers/operations.py`
- **Строки кода**: operations_flow.py — DeliveryDT amount опционально; при отсутствии/нуле amount_to_use = price × quantity; в ответе и в Operation сохраняется amount. operations.py — GET /operations возвращает amount в списке.
- **Что было**: amount обязательно; в результатах поиска операций поле «Сумма» могло не отображаться.
- **Что стало**: amount опционально; при создании без суммы подставляется цена × количество; в списке операций колонка «Сумма» заполняется из БД.
- **Проверка**: Создать выдачу без amount — в ответе и БД сумма = цена × количество; Поиск операций — в таблице отображается сумма.
...
- **Строки кода**: app.html — buildWriteOffForm, при сохранении createdBy берётся из currentUser.login (не window.currentUser), чтобы «Не определен текущий пользователь» не возникало; warehouse.py, operations_flow.py — без изменений по этому пункту.
- **Что было**: Ошибка «Не определен текущий пользователь» при сохранении списания.
- **Что стало**: created_by передаётся из текущего пользователя (currentUser после loadMe); списание сохраняется без ошибки.
- **Проверка**: Операции → Создать → Списание → выбрать склад → ввести количество → Сохранить — операции списания создаются.
...
- **Строки кода**: app.html — при сохранении перемещения createdBy берётся из currentUser.login (не window.currentUser), чтобы не было ошибки «Не определен текущий пользователь».
- **Что было**: Ошибка «Не определен текущий пользователь» при сохранении перемещения.
- **Что стало**: created_by передаётся из текущего пользователя; перемещение сохраняется без ошибки.
- **Проверка**: Операции → Создать → Перемещение между складами → выбрать склад от и склад в → ввести количества → Сохранить — операции создаются.

## Исправление #18: Раздача образцов — Реализация операции
- **Статус**: ✅ Исправлено
- **Файл(ы) изменены**: `src/static/app.html`
- **Строки кода**: при сохранении раздачи образцов createdBy берётся из currentUser.login (не window.currentUser). По бизнес-постановке операции раздачи образцов — уточнить отдельно.
- **Что было**: Ошибка «Не определен текущий пользователь» при сохранении раздачи образцов.
- **Что стало**: created_by передаётся из текущего пользователя; сохранение выполняется без ошибки.
- **Проверка**: Операции → Создать → Раздача образцов → выбрать экспедитора → склад от → ввести количество образцов → Сохранить — операции allocation создаются.
